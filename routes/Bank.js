	...
	if (err || bank == null) {
	  ...
	  data.save((err) => {
		...
		createWallet([bcode + '_operational@' + bank.name, bcode + '_master@' + bank.name]).then(function (result) {
		  let content = '<p>Your branch is added in E-Wallet application</p><p<p>&nbsp;</p<p>Login URL: <a href=\'http://' + config.mainIP + '/branch/' + bankName + '\'>http://' + config.mainIP + '/branch/' + bankName + '</a></p><p><p>Your username: ' + data.username + '</p><p>Your password: ' + data.password + '</p>'
