...
router.post("/bank/commission/updatePartnersShare", function (req, res) {
	const { token, commission_id, percentage, specific_partners_share } = req.body;
	Bank.findOne(
		{
			token,
			status: 1,
			...
			if (err) {
				...
				Commission.findOne({ _id: commission_id }, (err, comm) => {
					if (err) {
						...
						if ( comm.merchant_approve_status == 0 || comm.merchant_approve_status == 2 ) {
				...
				Commission.findOne(
					...
					(err, comm) => {
						if (err) {
							...
							if ( comm.infra_share_edit_status == 0 && comm.rule_edit_status == 1 ) {
								console.log("Condition 1");
								Commission.updateOne(
									{
										_id: commission_id,
							...
							} else if ( comm.infra_share_edit_status == 1 && comm.rule_edit_status == 1 ) {
								console.log("Condition 2");
								Commission.updateOne(
									{
										_id: commission_id,
									},
									{
										$set: {
											...
											"infra_share.percentage": comm.edited.infra_share.percentage,
											infra_share_edit_status: 0,
											rule_edit_status: 0,
											status: 1
										},
