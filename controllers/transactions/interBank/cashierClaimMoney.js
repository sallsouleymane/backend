...
module.exports = async function (
	...
	try {
		...
		distributeRevenue(transfer, sendingBank, bank, branch);
		return {
			status: 1,
...
async function distributeRevenue(transfer, sendingBank, bank, branch) {
	try {
		const bankOpWallet = bank.wallet_ids.operational;
		...
		if (transfer.claimerBankShare.percentage_amount > 0) {
			let trans2 = [
				{
					from: senderBankOpWallet,
					to: bankOpWallet,
					amount: transfer.claimerBankShare.percentage_amount,
...
async function transferToMasterWallets(
	...
	try {
		...
		const sendingBankOpWallet = sendingBank.wallet_ids.operational;
		const sendingBankMasterWallet = sendingBank.wallet_ids.master;
		...
		let claimBankPart = getPart(
			txInfo,
			master_code,
			[childType.INTER_BANK_PERCENT, childType.INTER_BANK_FIXED],
			[]
		);
		let claimBranchPart = getPart(txInfo, master_code, [childType.CLAIMER], []);
		let sendBankPart = getPart(
			txInfo,
			master_code,
			[childType.REVENUE],
			[
		...
		let trans = [
			{
				from: bankOpWallet,
				to: bankMasterWallet,
				amount: claimBankPart,
				note: "Claim Bank share",
				email1: bank.email,
				mobile1: bank.mobile,
				from_name: bank.name,
				to_name: bank.name,
		...
		trans = [
			{
				from: sendingBankOpWallet,
				to: sendingBankMasterWallet,
				amount: sendBankPart,
				note: "Sending Bank share",
				email1: infra.email,
				mobile1: infra.mobile,
				from_name: infra.name,
				to_name: infra.name,
...
function getAllShares(transfer, rule1, rule2) {
	...
	let claimerBankShare = calculateShare("claimBank", amount, rule1);
	let claimerBranchShare = 0;
	if (fee > 0) {
