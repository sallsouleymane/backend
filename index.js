...
console.log({ $gte: new Date(start), $lte: new Date(end) });
                BranchLedger.findOne({
                    created_at: { $gte: new Date(start), $lte: new Date(end) },
                    branch_id: user._id, trans_type: "CR"
                  },(e, post2) => {
                    ...
                    BranchLedger.findOne({
                      branch_id: user._id, trans_type: "DR",
                    created_at: { $gte: new Date(start), $lte: new Date(end) }
                  },(e, post3) => {
                          let paid = 0;
                      if(post3 != null && post3 != ''){
              ...
              data.save((err, d) => {
                if (err) return res.json({
                 ...
                 if(found == 1){
                  ...
                  console.log("Result: "+result);
                    if(result.length <= 0){
                      BranchSend.findByIdAndUpdate(d._id, {
                        status: 1,
                        fee: fee
                      }, (err) => {
                        if (err) return res.status(200).json({
                        ...
                        BranchLedger.findOne({ branch_id: f2._id, trans_type: "CR", created_at: {$gte: new Date(start), $lte: new Date(end)}}, function (err, c) {
                          if(err || c == null){
                            ...
                            var amt = Number(c.amount)+Number(oamount)+Number(fee);
                            BranchLedger.findByIdAndUpdate(c._id, {amount: amt}, function (err, c) {
              ...
              data.save((err, d) => {
                if (err) return res.json({
                  error: err.toString()
                    ...
                    if(result.length <= 0){
                      BranchClaim.findByIdAndUpdate(d._id, {
                        status: 1
                      }, (err) => {
                        if (err) return res.status(200).json({
                          error: err.toString()
                          ...
                          BranchLedger.findOne({ branch_id: f._id, trans_type: "DR", created_at: {$gte: new Date(start), $lte: new Date(end)}}, function (err, c) {
                          if(err || c == null){
